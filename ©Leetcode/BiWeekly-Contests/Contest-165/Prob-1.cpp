/* Smallest Absent Positive Greater Than Array */
/* 
...
public:
    int smallestAbsent(vector<int>& nums) {
        ...
        int candidate = std::max(1, static_cast<int>(floor(average)) + 1);
        
        std::unordered_set<int> present_nums(nums.begin(), nums.end());
        
        while (present_nums.count(candidate)) {
            candidate++;
        }
